...
const TOKEN_ADDRESSES: { [key: string]: string } = {
  USDC: '0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E', // AVAX USDC
  USDT: '0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7', // AVAX USDT
...
const GraphCanvas: React.FC = () => {
  ...
  const connectWallet = async () => {
    try {
      ...
      // Only check for MetaMask and Phantom
      if (ethereum?.isMetaMask && !ethereum?.isPhantom) {
        ...
        // If not MetaMask, proceed with WalletConnect
        console.log('Initializing WalletConnect...');
        const wcProvider = await EthereumProvider.init({
          projectId: 'da79bcd87753d60d76fcb54b8037de8d',
          chains: [43114],
          showQrModal: true,
        ...
        console.log('WalletConnect initialized, connecting...');
        ...
        // If no injected MetaMask, proceed with WalletConnect
        await wcProvider.connect();
        console.log('WalletConnect connected, getting accounts...');
            ...
            <div className="bg-[#2A2B32] rounded-lg p-4 shadow-lg min-w-[400px]">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input type="text" defaultValue="AVAX/USDT" className="w-full bg-gray-800 rounded px-2 py-1 text-white" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" defaultValue="1 AVAX" className="w-full bg-gray-800 rounded px-2 py-1 text-white" />
                </div>
                <div>
              ...
              <h3 className="text-white font-semibold mt-4 mb-3">Wallet Information</h3>
              <div className="space-y-3">
                <div>
                  <label className="text-gray-300 text-sm">Wallet Address</label>
                  <div className="bg-gray-800 rounded px-2 py-1 text-gray-400">0x123...</div>
                </div>
                <div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="bg-gray-800 rounded p-2">
                    <div className="text-white">AVAX: 11.5</div>
                    <div className="text-white">USDT: 455.75</div>
                  </div>
                </div>
                <div>
                  <label className="text-gray-300 text-sm">Price Feeds</label>
                  <div className="bg-gray-800 rounded p-2">
                    <div className="text-white">AVAX/USDT: 25.00</div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-[#2A2B32] rounded-lg p-4 shadow-lg min-w-[400px]">
              ...
              <div className="space-y-2">
                <div className="flex items-center">
                  <span className={`${node.data?.verificationStatus?.walletConnected ? "text-green-400" : "text-red-400"} mr-2`}>
                    {node.data?.verificationStatus?.walletConnected ? "✓" : "✗"}
                  </span>
                  <span className="text-white">Wallet connected</span>
                </div>
                <div className="flex items-center">
                  <span className={`${node.data?.verificationStatus?.tokenPairValid ? "text-green-400" : "text-red-400"} mr-2`}>
                    {node.data?.verificationStatus?.tokenPairValid ? "✓" : "✗"}
                  </span>
                  <span className="text-white">Token pair valid</span>
                </div>
                <div className="flex items-center">
                  <span className={`${node.data?.verificationStatus?.balanceCheck?.hasBalance ? "text-green-400" : "text-red-400"} mr-2`}>
                    {node.data?.verificationStatus?.balanceCheck?.hasBalance ? "✓" : "✗"}
                  </span>
                  <span className="text-white">Sufficient balance</span>
                </div>
                <div className="flex items-center">
                  <span className={`${node.data?.verificationStatus?.slippageCheck?.withinLimits ? "text-green-400" : "text-red-400"} mr-2`}>
                    {node.data?.verificationStatus?.slippageCheck?.withinLimits ? "✓" : "✗"}
                  </span>
                  <span className="text-white">Slippage within limits</span>
                </div>
              </div>
            </div>
          );
          ...
          <div className="bg-[#2A2B32] rounded-lg p-4 shadow-lg min-w-[400px]">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="bg-gray-800 rounded px-2 py-1 text-white">
                  {parseFloat(tradeConfig.tradeAmount) * 25} USDT
                </div>
              </div>
            </div>
            <h3 className="text-white font-semibold mt-4 mb-3">Wallet Information</h3>
                  ...
                  {node.type === 'rectangle' && nodes.filter(n => n.type === 'rectangle').indexOf(node) === 1 && (
                    <div className="bg-[#2A2B32] rounded-lg p-4 shadow-lg min-w-[400px]">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-gray-300">Token Pair Valid</span>
                          {node.data?.verificationStatus?.tokenPairValid ? 
                            <span className="text-green-500">✓</span> : 
                            <span className="text-red-500">✗</span>}
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-gray-300">Slippage Within Limits</span>
                          {node.data?.verificationStatus?.slippageCheck?.withinLimits ? 
                            <span className="text-green-500">✓</span> : 
                            <span className="text-red-500">✗</span>}
                        </div>
